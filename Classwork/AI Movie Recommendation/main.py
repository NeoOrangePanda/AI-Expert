...
try: df = pd.read_csv('Classwork\AI Movie Recommendation\IMDB top 1000.csv')
except FileNotFoundError: 
    print(Fore.RED + "Error! The file 'IMDB top 1000.csv' was not found."); raise SystemExit
...
def recommend(genre=None, mood=None, rating=None, n=5):
    d = df
    if genre: d[d['Genre'].str.contains(genre, case=False, na=False)]
    if rating is not None: d = d[d['Rate'] >= rating]
...
def get_genre():
    ...
    for i, g in enumerate(genres, 1): print(Fore.CYAN + f"{i}. {g}")
    print()
    while True:
